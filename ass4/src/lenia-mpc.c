...
#define EXCHANGED_SIZE (EXCHANGED_ROWS * SIZE)
#define MY_WORLD_SIZE (my_rows * SIZE)
#define MY_WORLD_TOTAL_SIZE ((my_rows + 2 * EXCHANGED_ROWS) * SIZE)
...
static inline void exchange_halo() {
    ...
    if (my_rows >= EXCHANGED_ROWS) {
        ...
        MPI_Sendrecv(
            // send:
            my_world + (my_rows - EXCHANGED_ROWS) * SIZE,
            EXCHANGED_SIZE,
            MPI_FLOAT,
            (myid + 1) % procs,
            1,
            ...
            MPI_COMM_WORLD,
            MPI_STATUSES_IGNORE);
    } else {  // our world is smaller than requested halo so we need data from more neighbors
        int needs = EXCHANGED_ROWS;
        int process_offset = 1;
        int bottom_filled = 0;
        int top_filled = EXCHANGED_ROWS;
        while (needs > 0) {
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    unsigned int cached_compute_rows = 0;
