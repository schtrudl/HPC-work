#!/usr/bin/env python3
"""Generate separate LaTeX tables for 1-node and 2-node MPI timings.

Usage:
    ./table_nodes.py timings_*_*_*.log > table_nodes.tex

The script expects filenames where the last two underscore-separated fields are:
    ..._<nodes>_<process_per_node>.log

Examples:
    timings_1_16.log
    timings_basic_row_2_32.log
"""
...
ParsedFile = Tuple[int, int, Dict[int, Entry]]  # (nodes, ppn, size->entry)
...
def parse_name(path: str) -> Tuple[int, int]:
    ...
    parts = stem.split("_")
    if len(parts) < 3:
        raise ValueError(f"cannot parse nodes/ppn from '{path}'")

    try:
        nodes = int(parts[-2])
        ppn = int(parts[-1])
    except ValueError as exc:
        raise ValueError(f"cannot parse nodes/ppn from '{path}'") from exc
...
def build_tabular(nodes: int, rows: List[ParsedFile]) -> List[str]:
    ppn_values = sorted({ppn for _, ppn, _ in rows})
    data_map = {(ppn): data for _, ppn, data in rows}

    # Keep sizes in ascending order across available files.
    sizes = sorted({size for _, _, data in rows for size in data})
    ...
    for size in sizes:
        row_entries = [
            data_map.get(ppn, {}).get(size)
            for ppn in ppn_values
            if data_map.get(ppn, {}).get(size) is not None
        ]
...
def build_side_by_side_table(grouped: Dict[int, List[ParsedFile]]) -> str:
    left_nodes = 1
    right_nodes = 2
    ...
    out.append(r"\caption{Execution time (seconds).}")
...
def main(paths: List[str]) -> int:
    ...
    for path in sorted(paths):
        try:
            nodes, ppn = parse_name(path)
        except ValueError:
            continue
        data = parse_log(path)
        if data:
            parsed.append((nodes, ppn, data))

    if not parsed:
        print("error: no parsable timing files provided", file=sys.stderr)
        return 1

    grouped: Dict[int, List[ParsedFile]] = {}
    for item in parsed:
        grouped.setdefault(item[0], []).append(item)

    print(build_side_by_side_table(grouped))
    return 0
